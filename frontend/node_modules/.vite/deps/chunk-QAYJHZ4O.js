...
var omit = flatRest_default(function(object, paths) {
  ...
  if (isDeep) {
    result2 = baseClone_default(result2, CLONE_DEEP_FLAG2 | CLONE_FLAT_FLAG2 | CLONE_SYMBOLS_FLAG2, customOmitClone_default);
  }
...
function throttleFilter(...args) {
  ...
  const filter2 = (_invoke) => {
    ...
    } else if (trailing) lastValue = new Promise((resolve, reject2) => {
...
function useEventListener(...args) {
  ...
  const firstParamTargets = computed(() => {
    const test = toArray2(toValue(args[0])).filter((e) => e != null);
  ...
  return watchImmediate(() => {
    ...
    return [
      ...
      toArray2(toValue(firstParamTargets.value ? args[1] : args[0])),
      toArray2(unref(firstParamTargets.value ? args[2] : args[1])),
      toValue(firstParamTargets.value ? args[3] : args[2])
    ];
  }, ([raw_targets, raw_events, raw_listeners, raw_options], _, onCleanup) => {
    if (!(raw_targets === null || raw_targets === void 0 ? void 0 : raw_targets.length) || !(raw_events === null || raw_events === void 0 ? void 0 : raw_events.length) || !(raw_listeners === null || raw_listeners === void 0 ? void 0 : raw_listeners.length)) return;
    const optionsClone = isObject3(raw_options) ? { ...raw_options } : raw_options;
...
function onClickOutside(target, handler, options = {}) {
  ...
  if (isIOS && !_iOSWorkaround) {
    ...
    Array.from(window2.document.body.children).forEach((el) => el.addEventListener("click", noop2, listenerOptions));
    window2.document.documentElement.addEventListener("click", noop2, listenerOptions);
  }
...
function useMutationObserver(target, callback, options = {}) {
  ...
  const stopWatch = watch(computed(() => {
    const items = toArray2(toValue(target)).map(unrefElement).filter(notNullish);
...
function onElementRemoval(target, callback, options = {}) {
  ...
  if (!window2 || !document2) return noop2;
  let stopFn;
...
function useElementBounding(target, options = {}) {
  ...
  useMutationObserver(target, update2, { attributeFilter: ["style", "class"] });
  if (windowScroll) useEventListener("scroll", update2, {
    capture: true,
    passive: true
  });
...
function useIntersectionObserver(target, callback, options = {}) {
  ...
  const targets = computed(() => {
    return toArray2(toValue(target)).map(unrefElement).filter(notNullish);
  });
  let cleanup = noop2;
...
var TransitionPresets = Object.assign({}, { linear: identity2 }, {
  easeInSine: [
    0.12,
    0,
    0.39,
...
function useWindowSize(options = {}) {
  ...
  if (window2 && type === "visual" && window2.visualViewport) useEventListener(window2.visualViewport, "resize", update2, listenerOptions);
  if (listenOrientation) watch(useMediaQuery("(orientation: portrait)"), () => update2());
  return {
    width,
    height
  };
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-QAYJHZ4O.js.map

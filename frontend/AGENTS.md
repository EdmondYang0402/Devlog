...
- 桌面前台 Navbar 默认隐藏，顶部保留吊绳/几何吊坠；仅 hover 或聚焦吊绳区域时下拉，从吊绳移动到 Navbar 时保持展开，离开两者后延迟收起。
- 吊绳不是点击或拖拽控件，也不以 hover 整个顶部触发。移动端必须有不依赖 hover 的方式。
...
- 音乐使用用户自己的公开网易云歌单，DevLog 只提供外层 UI，实际播放由网易云官方 iframe 完成；不存 MP3、不调用非官方网易云 API、不操作跨域 iframe 内部 DOM。
- 当前播放器仅在首页左侧，默认折叠、点击展开，不是跨路由常驻播放器。只有用户明确改变该长期设计时才调整。
